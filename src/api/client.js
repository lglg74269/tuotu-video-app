...
export const Settings = {
  ...
  save: async (s) => { 
    ...
    await Storage.writeSettings(plainS); 
    return plainS; 
  },
  test: async () => {
...
export const Pipeline = {
  ...
  analyze: async (b, signal) => await analyzeScript(b),
  splitStoryboard: async (b, signal) => await splitEpisodeText(b),
  splitChunk: async (b, signal) => await splitSingleChunk(b),
  segmentText: async (b, signal) => ({ chunks: segmentEpisodeText(b.episodeText, b.maxUnitChars) }),
  createShots: async (b, signal) => await createShotsForUnits(b),
  mergeStoryboards: async (parts, signal) => mergeStoryboards(parts || []),
  validateSb: async (b, signal) => { throw new Error('Not implemented'); },
  classify: async (b, signal) => await classifyShots(b),
