...
export async function splitChunk(args) {
  ...
  const { content, parsed } = await callLLM(prompt, { expectJson: true, nodeKey });
  const meta = {
  ...
  await runPool(batches, 4, async (batch) => {
    try {
      ...
      const { content, parsed } = await callLLM(
        userContent,
        { system: systemPrompt, responseFormatJson: true, expectJson: true, temperature: 0.1, maxTokens: 4096, retries: 3, nodeKey: 'step3b-split-script' }
      );
      allPayloads.push({
